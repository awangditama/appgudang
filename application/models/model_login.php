...
class Model_login extends CI_Model {
    ...
    function check_login($username, $password) {
     ...
     function ambil_driver() {
        $this->db->where("nm_jabatan", "Driver");
          $query = $this->db->get("v_kar_jab");
        if ($query->num_rows() > 0) {
           return $query->result();
        } else {
            echo "false";
    ...
    function add_pinjam_mobil($nik,$hari,$jam_mulai,$jam_akhir,$tujuan,$keperluan) {
        $this->db->set('nik', $nik);
        $this->db->set('hari', $hari);
        $this->db->set('jam_mulai', $jam_mulai);
        $this->db->set('jam_akhir', $jam_akhir);
        $this->db->set('tujuan', $tujuan);
        $this->db->set('keterangan', $keperluan);
        
        $this->db->insert('tbl_pinjam_mobil');
        return true;
    }
